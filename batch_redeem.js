...
async function execSafeTransaction(signer, safeContract, to, data, provider) {
  ...
  const domain = {
    verifyingContract: await safeContract.getAddress(),
...
async function main() {
  ...
  const signer = new ethers.Wallet(
    process.env.POLYSNIPER_PRIVATE_KEY,
    provider,
  );
  ...
  for (let i = 0; i < positions.length; i++) {
    ...
    try {
      ...
      // 构造 Redeem 数据
      const redeemCalldata = ctf.interface.encodeFunctionData(
        "redeemPositions",
        [USDC_ADDRESS, ethers.ZeroHash, pos.conditionId, [indexSet]],
      );
  ...
  const usdcTokenId = ethers.keccak256(
    ethers.AbiCoder.defaultAbiCoder().encode(["address"], [USDC_ADDRESS]),
  );
  ...
  if (ctfBalance > 0n) {
    ...
    const withdrawCalldata = ctf.interface.encodeFunctionData(
      "safeTransferFrom",
      [
        PROXY_WALLET_ADDRESS,
        PROXY_WALLET_ADDRESS,
